...
### 1. Data definition language untuk pembuatan tabel.
#### CREATE
Membuat skema `public`
```sql
CREATE SCHEMA public;
```

Membuat enum-enum yang akan dibutuhkan pada saat pembuatan tabel-table
```sql
...
```sql
DROP TABLE coupon_customers;
```

```sql
DROP TABLE coupon_stores;
```
...
```sql
DROP TABLE item_addons;
```

```sql
DROP TABLE item_addon_categories;
```
...
```sql
DROP TABLE item_category_l10ns;
```
...
```sql
DROP TABLE item_sub_category_l10ns;
```
...
```sql
DROP TABLE order_detail_addons;
```
...
```sql
DROP TABLE store_staffs;
```
#### ALTER TABLE
Menambahkan foreign key untuk tabel yang sudah dibuat
...
```
### Data manipulation language untuk contoh data awal.
#### INSERT
...
```sql
INSERT INTO item_sub_categories (id, name)
VALUES (
...
```
#### UPDATE
Mengubah beberapa data pada setiap tabel
```sql
UPDATE coupons
